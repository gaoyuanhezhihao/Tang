...
class CarAdmin():
    ...
    def step_left(self):
        ...
        self.Send_Direct_Order(order='l', data1=self.steps/256,
                               data2=self.steps % 256)

    def step_right(self):
        self.steps = int(self.steps_entry.get())
        print "steps:%s" % self.steps_entry.get()
        self.Send_Direct_Order(order='r', data1=self.steps/256,
                               data2=self.steps % 256)
    ...
    def serial_ports(self):
        ports = [serial_prefix + str(i) for i in range(256)]
        result = []
        for port in ports:
            try:

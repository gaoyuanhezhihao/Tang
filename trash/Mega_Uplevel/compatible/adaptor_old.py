...
import socket
import serial
from Calibrate_win7 import CarAdmin
...
order_map = {'l':'y', 'r':'z'}
order_remap = {'y':'l', 'z': 'r'}
micro_move_order_map = {'m':'K', 'n':'J', 'x':'H', 'y':'L'}
micro_move_ack_map = {'K': 'm', 'J':'n', 'H':'x', 'L':'y'}
            ...
            if len(self.msg_list) > 1:
                ...
                for m in self.msg_list[:-1]:
                    ...
                    if m in Valid_replies:
                        if self.micro_move_mode:
                            if m[0] in 'mnxy':
                                m = micro_move_ack_map[m[0]] + m[1:]
                        else:
                            if m[0] in ['y', 'z']:
                                m = order_remap[m[0]] + m[1:]
            ...
            while True:
                try:
                    ...
                    if command_tokens[0] in ['f', 's', 'b']:
                        ...
                        self.Send_Direct_Order(order = order_map[command_tokens[0]], data1=pulses/256, data2=pulses%256)

                    if command_tokens[0] in "mnxy" and len(command_tokens) >= 2:
                        self.micro_move_mode = True
                        time_to_push = int(command_tokens[1])
                        new_order = micro_move_order_map[command_tokens[0]]
                        self.Send_Secret_Order(order = new_order, data1=time_to_push/256, data2=time_to_push%256)
                    else:
                        self.micro_move_mode = False
